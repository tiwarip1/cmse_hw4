...
class Spartan(random_walk):
    ...
    def kill_buckeye(self):
        '''If there is a buckeye nearby, there is a random chance the spartan
            will kill the wolverine and add that to the amount of food it has'''
        chance_kill = random.uniform(0,100)
        killed = False
        if chance_kill<75:
...
class Buckeye(random_walk):
    ...
    def step(self):
        ...
        if self.food>1:
            self.buckeye_reproduce()

    def buckeye_killed(self):
        '''Gets rid of Buckeye from the grid'''
        self.model.grid._remove_agent(self.pos,self)
        self.model.schedule.remove(self)

    def food_function(self):
        '''If a Buckeye eats a wolverine, its food will increase'''
        self.food+=1
    ...
    def buckeye_reproduce(self):
        '''If a Buckeye eats enough wolverines, it will take a step to stop
        and produce another buckeye in the same cell'''
        self.food = self.food/2
        cub = Spartan(self.pos, self.model)
        self.model.grid.place_agent(cub, cub.pos)
        self.model.schedule.add(cub)
        print("A Buckeye was born")
    ...
    def old_age(self):
        '''There is a random chance the Buckeye will die in this step'''
        is_alive = random.uniform(0,100)
        if is_alive>95:
            self.alive = False
            print("Buckeye died of old age")
